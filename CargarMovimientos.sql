...
BEGIN TRY
    ...
    BEGIN
        ...
        -- Traducir Cédula a IDEmpleado y Username a IDUsuario
        ...
        BEGIN
            PRINT CONCAT('No se encontró Empleado (', @DocIdentidad, ') o Usuario (', @Username, '). Omitiendo movimiento.');
        END
        ELSE
        BEGIN
            -- llamar SP
            ...
            BEGIN
                ...
                PRINT CONCAT('error', @OutResult, ' (', ISNULL(@ErrorMsg, 'Descripción no encontrada'), '). Omitiendo movimiento.');
            END
        END
...
END TRY
BEGIN CATCH
    -- Este CATCH ahora solo se activará si hay un error
